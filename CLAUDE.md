...
- **CSP labels**: `fixed`, `investments`, `shrinking`, `guilt-free`, `income`. The mapping from Monarch category → CSP label is stored per-user in Firestore config (`csp_from_group`, `csp_from_category`, `csp_labels`). Note: the `shrinking` label (sinking funds for near/midterm goals — gifts, vacations, home improvements) is distinct from the Monarch "Savings" category, which is a csp *key* (`savings`) that maps to the `investments` label (money moved into long-term investment accounts).
