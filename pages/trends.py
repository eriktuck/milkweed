...
_GROUP_ORDER = ['income', 'fixed', 'investments', 'savings', 'guilt-free']
...
@callback(
    ...
    Output('trends-filter-savings', 'value', allow_duplicate=True),
    ...
    State('trends-filter-savings', 'options'),
...
@callback(
    ...
    Output('trends-filter-savings', 'options'),
    Output('trends-filter-savings', 'value'),
...
@callback(
    ...
    Input('trends-filter-savings', 'value'),
...
def update_trends_chart(transactions_data, start_date, end_date, smoothing,
                         ...
                         f_income, f_fixed, f_investments, f_savings, f_guilt_free):
    if not transactions_data or not use_case:
        raise PreventUpdate

    transactions = pd.read_json(StringIO(transactions_data), orient='split')

    selected = (f_income or []) + (f_fixed or []) + (f_investments or []) + (f_savings or []) + (f_guilt_free or [])
    if selected:
